...
{
	int ctrl_menu,c_menu, ps_menu, lw_menu;

	ctrl_menu = glutCreateMenu(ctrlMenu);
	glutAddMenuEntry("F1 for black background", 123);
	glutAddMenuEntry("F2 for white background", 123);
	glutAddMenuEntry("F3 for dark red background", 123);
	glutAddMenuEntry("F4 for cyan background", 123);
	glutAddMenuEntry("1 for point mode ", 123);
	glutAddMenuEntry("2 for line mode", 123);
	glutAddMenuEntry("3 for wireframe triangle mode", 123);
	glutAddMenuEntry("4 for filled triangle mode", 123);
	glutAddMenuEntry("5 for wireframe rectangle mode", 123);
	glutAddMenuEntry("6 for filled rectangle mode", 123);
	glutAddMenuEntry("7 for wireframe circle mode", 123);
	glutAddMenuEntry("8 for filled circle mode", 123);
	...
	glutAddSubMenu("Controls" , ctrl_menu);
...
// Select background
void selectBackground(int key, int x , int y) {
	switch(key) {
		case GLUT_KEY_F1:
			bkColor.r = 0;
			bkColor.g = 0;
...
// Select drawing mode from keyboard
void selectModeWithKeyboard(unsigned char key , int x , int y) {
	switch(key) {
		case '1' :
			mode = POINT_MODE;
			break;
...
int main(int argc, char **argv)
{
	...
	glutSpecialFunc(selectBackground);
	glutKeyboardFunc(selectModeWithKeyboard);
	glutMainLoop();
